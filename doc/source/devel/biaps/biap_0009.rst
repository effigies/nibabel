...
Additionally, CIFTI-2 permits subsets of surfaces to be sampled by explicitly listing
the coordinate indices.
Therefore we need the ability to load pre-masked data, which for meshes means a list of
vertices, potentially in a different order to the reference coordinates.

.. code-block:: python

    def select_parcel():
        img = nb.load("sub-01_task-rest_bold.dtseries.nii")  # Assume CIFTI SurfaceImage
        parcel = nb.load("sub-fsLR_hemi-L_label-DLPFC_mask.label.gii")
        structure = parcel.get_structure()  # Does not exist
        vtx_idcs = np.where(parcel.agg_data())[0]
        header, slicer = img.header.slice(structure=structure, indices=vtx_idcs)
        # new header will return a corresponding geometry
        patch_img = img.__class__(img.dataobj[slicer], header=header)
        patch_img.load_geometry("fsLR.wb.spec")


Considerations

  * Should ``SurfaceHeader`` hold onto geometries loaded by the current image?
    This would allow them to validate newly-loaded geometries. Alternately, the
    ``SurfaceHeader`` provides access to an in-built geometry and a potential
    validation method, but the ``SurfaceImage`` holds onto actual geometries
